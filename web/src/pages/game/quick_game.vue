<template>
    ...
    >
        <template v-slot:text>
            <v-row align="center" class="mb-4" no-gutters>
                ...
                <v-col cols="5" class="d-flex justify-end align-center">
                    ...
                    >
                        <v-card>
                            <v-card-title>添加rpk小游戏</v-card-title>
                            <AddGame />
                        </v-card>
                        </v-dialog>
                    <v-btn
        ...
        <v-dialog
            ...
            max-width="600px">
            <v-card>
                <v-card-title>编辑rpk小游戏</v-card-title>
...
<script setup lang="ts" name="QuickMiniGame">
import { ref, onMounted } from 'vue';
import { searchGame, type SearchGame, type Game, findRpkGame } from '@/api/game';
import AddGame from '@/components/game/add_game.vue';
import UpdateGame from '@/components/game/update_game.vue';

const searchName = ref('');
const case_type = ref('default');
const games = ref([]);
...
const refreshGames = async () => {
    ...
    try {
        console.log('刷新游戏列表');
        searchName.value = '';
        case_type.value = 'default';
        const res:any = await findRpkGame();
        if (res.status) {
